...
{
    cfg.AddPolicy(PolicyDefaults.AdminName, options => options.RequireRole(RoleDefaults.Admin, RoleDefaults.SuperAdmin));

    cfg.AddPolicy(PolicyDefaults.PaidAndAdminName, options => options.RequireRole(RoleDefaults.Medium, RoleDefaults.Premium, RoleDefaults.Admin, RoleDefaults.SuperAdmin));

    cfg.AddPolicy(PolicyDefaults.ConfirmedAccount, options => options.RequireClaim("confirmed", "True"));
});
...
builder.Services.AddCors(options => options.AddPolicy("main-cors", policy =>
    {
        policy
            .AllowAnyMethod()
            .AllowAnyHeader()
